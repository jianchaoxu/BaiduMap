...
{
    self.statusView.view.frame = CGRectMake(20, DeviceHeight - 307, 338, 300);
}
...
- (void)setupNavigationProperty
{
    ...
    // 导航栏左侧按钮
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"开始"
                                                                            style:UIBarButtonItemStylePlain
                                                                           target:self
                                                                           action:@selector(startTrack)];
    // 导航栏右侧按钮
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"结束"
                                                                             style:UIBarButtonItemStylePlain
                                                                            target:self
...
- (void)didUpdateBMKUserLocation:(BMKUserLocation *)userLocation
{
    ...
    ///4.geo搜索服务 (将经纬度转化为地址,城市等信息,被称为反向地理编码)
...
-(void) onGetReverseGeoCodeResult:(BMKGeoCodeSearch *)searcher result:(BMKReverseGeoCodeResult *)result errorCode:(BMKSearchErrorCode)error
{
    if (error == BMK_SEARCH_NO_ERROR) {
        ...
        NSLog(@"测试结果 %@  %@  %@  %@",result.addressDetail.city,result.addressDetail.streetName,result.addressDetail.streetNumber,result.address);
        self.statusView.locationResult.text = result.address ;
    }
    else {
        NSLog(@"抱歉，未找到结果");
    }
